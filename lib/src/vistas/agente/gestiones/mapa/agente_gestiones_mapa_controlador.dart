...
class AgenteGestionesMapaController {
  ...
  //Método para actualizar estado concretado
  void updateToConcretado() async {
    if (_distanceBetween <= 200) {
      _progressDialog.show();
      ResponseApi responseApi =
          await _ordersProvider.updateTicketToConcretado(order);

      if (responseApi.success) {
        ReportsHasReports reportsHasReports = new ReportsHasReports(
          idReports:
              '${_idreports.idReportConcretado.toString()}', // ID del tipo de reporte para las órdenes canceladas
          idAgent: '${idUserAgente}',
          nameReport:
              'Orden Concretada\n Id Order: ${order.id},\n (Agente)\n ${order.agent.name}, ${order.agent.lastname}',
          descriptionReport:
              'Se concretó la orden con ID: ${order.id},La orden fue concretada por el Agente: ${order.agent.name}, ${order.agent.lastname},\n\n ===  Datos del Agente === \n\n ID: ${order.agent.id},\n Nombre: ${order.agent.name},\n Apellidos: ${order.agent.lastname},\n E-mail agente: ${order.agent.email},\n Celular agente:${order.agent.phone},\n \n === Datos de Cliente ===\n \n Nombre: ${order.client.name},\n Apellidos: ${order.client.lastname},\n E-mail: ${order.client.email},\n\n  === Datos de Orden === \n\n id: ${order.id},\n Fecha: ${order.timestamp},\n Lugar de la Cita: ${order.address.address}\n\n',
        );
        ...
        Future.delayed(Duration(seconds: 2), () {
          Navigator.pushNamedAndRemoveUntil(
              context, 'agente/propiedades/list', (route) => false);
        });
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'Para Concretar la orden debes estar en el punto de encuentro'),
        behavior: SnackBarBehavior.floating,
        backgroundColor: Colors.red[600],
      ));
      return;
  ...
  //Método para actualizar estado cancelado
  ...
  void updateOrderCancel() async {
    ...
    if (responseApi.success) {
      ...
      ReportsHasReports reportsHasReports = new ReportsHasReports(
        ...
        descriptionReport:
            'Se ha cancelado la orden con ID: ${order.id},La orden fue cancelada por el Cliente: ${order.client.name}, ${order.client.lastname},\n\n=== Datos de Cliente ===\n \n Nombre: ${order.client.name},\n Apellidos: ${order.client.lastname},\n E-mail ${order.client.email},\n\n  === Datos de Orden === \n\n id: ${order.id},\n Fecha: ${order.timestamp},\n Lugar de la Cita:${order.address.address} \n\n',
      );
      ResponseApi reportResponse =
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Se ha concretado la orden: ${responseApi.message}'),
        behavior: SnackBarBehavior.floating,
        backgroundColor: Colors.lightGreen[900],
      ));
  ...
  void launchWaze() async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      MySnackbar.show(context, 'Waze no puede Inicializarse');

      print('La Aplicación Waze no esta instalada');
    }
  }

  void launchGoogleMaps() async {
// Verifica si la aplicación de Google Maps está instalada en el dispositivo del usuario.
    final uri = Uri.parse(
        'google.navigation:q=${order.address.lat.toString()},${order.address.lng.toString()}');
    if (await canLaunchUrl(uri)) {
// La aplicación de Google Maps está instalada.
      await launchUrl(uri);
    } else {
// La aplicación de Google Maps no está instalada.
// Abre el sitio web de Google Maps en el navegador web predeterminado del usuario.
  ...
  void llamarCliente() {
    final uri = Uri.parse("tel://${order.client.phone}");
    launchUrl(uri);
  }
  ...
  void updateLocation() async {
    try {
      ...
      _positionStream = Geolocator.getPositionStream(
        ...
        _position = position;
        emitPosition();
        addMarker('Agente Inmobiliario', _position.latitude,
            _position.longitude, 'Tu Posición', '', agentMarker);
        animateCamaraToPosition(_position.latitude, _position.longitude);
        isCloseToAgentePosition();
        refresh();
      });
    } catch (error) {
      print(
