...
import 'cliente_ordenes_detalle_controlador.dart';

class ClienteOrdenesDetallePage extends StatefulWidget {
  Order order;
  ClienteOrdenesDetallePage({Key key, @required this.order}) : super(key: key);

  @override
  State<ClienteOrdenesDetallePage> createState() =>
      _ClienteOrdenesDetallePageState();
}

class _ClienteOrdenesDetallePageState extends State<ClienteOrdenesDetallePage> {
  ClienteOrdenesDetalleController _controlador =
      new ClienteOrdenesDetalleController();
...
//Método Con información de cliente
  ...
  Widget _infoCliente() {
    final clientName = _controlador.order?.agent?.name ?? 'Sin nombre';
    final clientLastName =
        _controlador.order?.agent?.lastname ?? 'Sin Apellido';
    final clientPhone = _controlador.order?.agent?.phone ?? 'Sin Teléfono';
    final clientEmail = _controlador.order?.agent?.email ?? 'Sin Email';
    ...
    if (_controlador.order != null &&
        _controlador.order.address.dateEvent != null) {
      formattedDate =
